...
class halo_props:
    ...
    def calcu_radii_masses(self, halo_id_list=[], rdict=None, precision=1e-2, rmax=None):
        ...
        for j in halo_id_list:
            # i = j - 1
            i = np.nonzero(self.haloid == j)[0][0]
            prop = self.dict[i]
            t1 = time.time()
    ...
    def get_galaxy(self, g_low_limit, mode='only stellar'):
        ...
        # def mstar_msfgas_toPool(i):
        #     j = self.haloid[i]
        #     tot_star = self.new_catalogue[j].star['mass'].sum()
        #     sf_gas = self.new_catalogue[j].gas[pnb.filt.LowPass('temp', '3e4 K')]
        #     tot_sfgas = sf_gas['mass'].sum()
        #     return j, tot_star, tot_sfgas
        
        # with Pool(self.nthreads) as pool:
        #     if self.verbose:
        #         msmsf = np.array(list(tqdm.tqdm(pool.imap_unordered(mstar_msfgas_toPool, range(self.length)), total=self.length)))
        #     else:
        #         msmsf = np.array(pool.imap_unordered(mstar_msfgas_toPool, range(self.length)))

        # sort_idx = np.argsort(msmsf[:, 0])
        # self.prop['M']['total_star'] = pnb.array.SimArray(msmsf[sort_idx, 1], 'Msol')
        # self.prop['M']['total_sfgas'] = pnb.array.SimArray(msmsf[sort_idx, 2], 'Msol')
        ...
        # def self_mstar_msfgas_toPool(i):
        #     j = self.haloid[i]
        #     if len(self.children[i]) == 0:
        ...
        #     else:
        #         children_union = get_union(self.new_catalogue, list(self.children[i]))
        ...
        #     return j, self_Mstar, self_Msfgas
        
        # with Pool(self.nthreads) as pool:
        #     if self.verbose:
        #         selfmsmsf = np.array(list(tqdm.tqdm(pool.imap_unordered(self_mstar_msfgas_toPool, range(self.length)), total=self.length)))
        #     else:
        #         selfmsmsf = np.array(pool.imap_unordered(self_mstar_msfgas_toPool, range(self.length)))
        # sort_idx = np.argsort(selfmsmsf[:, 0])
        # self.prop['M']['self_star'] = pnb.array.SimArray(selfmsmsf[sort_idx, 1], 'Msol')
        # self.prop['M']['self_sfgas'] = pnb.array.SimArray(selfmsmsf[sort_idx, 2], 'Msol')
